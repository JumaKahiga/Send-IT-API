...
class TestUser(BaseTest):
    ...
    def test_new_user(self):
        respo = self.client.post(
            '/auth/signup', data=json.dumps(self.sample_user), content_type='application/json')
        ...
        self.assertEqual(self.sample_user['username'], result['data']['username'])

    def test_login_user(self):
        respo = self.client.post(
            '/auth/login', data=json.dumps(self.sample_login), content_type='application/json')
        result = json.loads(respo.data.decode())
        self.assertEqual(respo.status_code, 201)
